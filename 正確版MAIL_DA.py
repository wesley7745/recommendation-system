...
# 讀取Excel文件
# =============================================================================
file_pro = pd.read_excel("C:/Users/WCHuang8/Desktop/學習推薦系統專案/word2vec/output/top_matches_TDA_DS_translated.xlsx")
file_AI = pd.read_excel("C:/Users/WCHuang8/Desktop/學習推薦系統專案/word2vec/output/top_matches_TDA_AI3_translated.xlsx")

# 部門列表和收件人對應
departments_receivers = {
    'Digital Platform Strategy and Promotion Department': ['wchuang8@winbond.com'],
    'AI Strategy and Promotion Department': ['wchuang8@winbond.com'],
    'Data Analysis Platform and Promotion Department': ['wchuang8@winbond.com'],
    'Application System Quality, Security, and Architecture Department ': ['wchuang8@winbond.com']
}

# 部門與主管名稱對應
department_managers = {
    'Digital Platform Strategy and Promotion Department': 'DA10主管',
    'AI Strategy and Promotion Department': 'DA20主管',
    'Data Analysis Platform and Promotion Department': 'DA30主管',
    'Application System Quality, Security, and Architecture Department ': 'DA40主管'
}

# 圖像路徑字典
images = {
    'Digital Platform Strategy and Promotion Department': "C:/Users/WCHuang8/Desktop/picture/digital.png",
    'AI Strategy and Promotion Department': "C:/Users/WCHuang8/Desktop/picture/AI.png",
    'Data Analysis Platform and Promotion Department': "C:/Users/WCHuang8/Desktop/picture/data.png",
    'Application System Quality, Security, and Architecture Department ': "C:/Users/WCHuang8/Desktop/picture/application.png"
}
...
#     # 設置每個部門特定的主管名稱
#     # manager_name = department_managers.get(department, "主管")
#     # header = f"<h3>Dear {manager_name}：</h3><p>向您推薦一些優質的學習資源，這些資源可以幫助您和您的團隊在專業方面進一步提升。以下是外部學習資源系統判斷出非常有價值的課程和工具：</p>"
#     # full_body_start = css + header
#     manager_name = department_managers.get(department, "主管")
#     header = f'<p class="MsoNormal"><span style="font-family:&quot;微軟正黑體&quot;,sans-serif"><h3>Dear {manager_name}：</h3><p>向您推薦一些優質的學習資源，這些資源可以幫助您和您的團隊在專業方面進一步提升。以下是外部學習資源系統判斷出非常有價值的課程和工具：</p></span></p>'
#     full_body_start = css + header
...
#         image_tag = f'<p><img src="cid:{image_cid}" width=100% height=100%></p>'
#         full_body += image_tag
    
#     # 添加部門的專業課程
#     if not dept_file_pro.empty and any(dept_file_pro['choose'] == 1):
#         body = f"<p>推薦給{department}的 <strong>專業</strong> 線上課程：</p>"
#         body += "<table><tr><th>課程</th><th>課程描述</th><th>課程等級</th></tr>"
#         for i in range(len(dept_file_pro)):
#             if dept_file_pro['choose'].iloc[i] == 1:
#                 body += "<tr>"
#                 body += f'<td><a href="{dept_file_pro["url"].iloc[i]}">{dept_file_pro["Course Title"].iloc[i]}</a></td>'
#                 body += f"<td>{dept_file_pro['Course Description 中文'].iloc[i]}</td>"
#                 body += f"<td>{dept_file_pro['course_level'].iloc[i]}</td>"
#                 body += "</tr>"
#         body += "</table>"
#         full_body += body
...
#     # 添加部門的AI課程
#     if not dept_file_ai.empty and any(dept_file_ai['choose'] == 1):
#         body = f"<p>推薦給{department}的 <strong>AI</strong> 線上課程：</p>"
#         body += "<table><tr><th>課程</th><th>課程描述</th><th>課程等級</th></tr>"
#         for i in range(len(dept_file_ai)):
#             if dept_file_ai['choose'].iloc[i] == 1:
#                 body += "<tr>"
#                 body += f'<td><a href="{dept_file_ai["url"].iloc[i]}">{dept_file_ai["Course Title"].iloc[i]}</a></td>'
#                 body += f"<td>{dept_file_ai['Course Description 中文'].iloc[i]}</td>"
#                 body += f"<td>{dept_file_ai['course_level'].iloc[i]}</td>"
#                 body += "</tr>"
#         body += "</table>"
#         full_body += body
